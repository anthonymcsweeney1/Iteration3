...
try {
    // prepare select query
    $query = "Update claims SET Status ='Approved' Where ClaimID = $id;" ;  echo "<script>alert('Claim is now approved.  Thank you.')</script>";
...
$mail->Password = 'FYP2021!';  // password
 $mail->FromName = 'For Your Information: Claim Settlement '.$row1['ClaimNum'].' has been approved for '.$row1['Currency'].''.$row1['amount'].' .';    //Sets the From name of the message
...
$mail->Subject = 'For Your Information: Claim Settlement '.$row1['ClaimNum'].' has been approved for '.$row1['Currency'].''.$row1['amount'].' .'; 
...
if(!$mail->send()){
    ...
    ?>
<htm>
    <a href="ApproverClaims.php" >Go back to Claim page</a>
